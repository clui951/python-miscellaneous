class Solution:
    ...
    # space is O(3^N) where N is the length of digits, because we build up a list of the combinations; we need to track N generators
    # time is O(#^N) also, because we generate each combination; every index, we only call the helper once, and yield O(1), 3 or 4, elements per call
